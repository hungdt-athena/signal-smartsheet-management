...
- Rendering: if `screenshot_urls` non-empty → existing StoreKit card, unchanged. Else → **Manual Screenshots card**:
  ...
  - Dropzone (visible to the same users; hidden when storage is unconfigured — detected by a 503 on save, and the dropzone shows the error state): "Dán ảnh (Ctrl+V), kéo thả, hoặc bấm để chọn". A `paste` listener on `window` is attached only while the card is mounted (and only for users who can edit) and ignores events when focus is in an input/textarea. Pasting while the lightbox is open merely stages files — harmless, so no extra guard.
